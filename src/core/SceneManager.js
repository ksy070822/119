/**
 * 씬 전환 (v3): title → intro → game → magicVideo → boss → ending
 * v2 레거시 씬 (VillageScene, ControlCenterScene) 제거됨
 */
import { TitleScene } from '../scenes/TitleScene.js';
import { IntroScene } from '../scenes/IntroScene.js';
import { GameScene } from '../scenes/GameScene.js';
import { MagicVideoScene } from '../scenes/MagicVideoScene.js';
...
export class SceneManager {
  ...
  async init() {
    ...
    this.scenes.magicVideo = new MagicVideoScene(this.game);
    ...
    await this.scenes.magicVideo.init?.();
  ...
  async goTo(name) {
    ...
    this.game.bgm?.playForScene(name);
  }
}
